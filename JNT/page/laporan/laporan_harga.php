...
<hr>
   <h3><center><b>LAPORAN HARGA ASET PADA 
    <?php
      if($bulan1 != ''){
...
<table border="1" width="100%" style="border-collapse: collapse;">
   ...
   <th width="25px" height="50px">Harga Aset</th>
    ...
    <tbody>
        ...
        INNER JOIN brg_masuk_peralatan using(id_pengajuan) 
        WHERE $where 
        AND pengajuan_dtl_mmsk.status='DITERIMA'
        ...
        ?>
          <tr>
              ...
              <td width="100px" height="50px"><center><?php echo format_indo(date($data['tgl_diterima']));?></center></td>
        ...
        <?php   } ?>
            ...
            <?php $bua = mysqli_query($koneksi,"SELECT count(id_pengajuan) as total
              ...
              WHERE $where
              ORDER BY pengajuan_msk.tgl_masuk Asc ");
               $tot = mysqli_query($koneksi,"SELECT SUM(pengajuan_dtl_mmsk.harga) total_harga
              from  pengajuan_msk
        INNER JOIN pengajuan_dtl_mmsk using(id_pengajuan_msk) 
        INNER JOIN kategori_brg using(id_kategori) 
        INNER JOIN brg_masuk_peralatan using(id_pengajuan) 
        WHERE $where 
        AND pengajuan_dtl_mmsk.status='DITERIMA'
        ORDER BY kategori ASC ")or die($koneksi->error);?>
